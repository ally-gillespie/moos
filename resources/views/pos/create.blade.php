...
<div class="max-w-3xl mx-auto p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <form method="POST" action="{{ route('kitchen.logout') }}">
            @csrf
            <button class="text-sm text-neutral-500 underline">Log out</button>
        </form>
    </div>

    <form id="pos-form" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="customer_name" maxlength="60" class="w-full border rounded-lg px-3 py-2">
        </div>
        ...
        <div>
            ...
            <div class="flex gap-4">
                <label class="flex items-center gap-2">
                    <input type="radio" name="payment_method" value="card" checked> Card (SumUp reader)
                </label>
                <label class="flex items-center gap-2">
        ...
        <div id="meal-deal-banner" class="hidden text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg px-4 py-2">
            Meal Deal — <span id="meal-deal-saving"></span> off
        </div>

        <div class="flex items-center justify-between border-t pt-4">
            <span class="text-xl font-semibold">Total: <span id="total-display">£0.00</span></span>
            <button type="submit" class="bg-black text-white px-6 py-3 rounded-lg font-medium hover:bg-neutral-800">
                Confirm Order
            </button>
        </div>
    </form>

    <div id="result" class="hidden mt-6 p-6 bg-white border rounded-xl text-center">
        <p class="text-neutral-500">Order number</p>
        <p id="result-number" class="text-6xl font-bold"></p>
    </div>
</div>
...
<template id="item-template">
    <div class="cart-item border rounded-lg p-4 bg-white space-y-3">
                ...
                @foreach($menu['categories'] as $cat)
                    <optgroup label="{{ $cat['label'] }}">
                        @foreach($cat['items'] as $item)
                            <option value="{{ $item['key'] }}" data-name="{{ $item['name'] }}"
                                    data-price="{{ $item['price_pence'] }}" data-type="{{ $cat['key'] }}"
        ...
        <div class="burger-customise">
            ...
            <div class="flex flex-wrap gap-2 mt-2 paid-extras">
...
document.getElementById('pos-form').addEventListener('submit', async (e) => {
    ...
    const response = await fetch('{{ route('pos.store') }}', {
        ...
        body: JSON.stringify({ items, customer_name: customerName, payment_method: paymentMethod }),
    });

    if (response.ok) {
        const data = await response.json();
        document.getElementById('result-number').textContent = data.order_number;
